...
if [ ! -f binutils-2.32.tar.gz ];
then
	wget http://ftp.gnu.org/gnu/binutils/binutils-2.32.tar.gz
fi
tar xvfz binutils-2.31.tar.gz
../binutils-2.32/configure --target=$TARGET --prefix="$PREFIX" --with-sysroot=$CROSS_SYSROOT --disable-werror

# rm -rf gcc-7.1.0
if [ ! -f gcc-8.3.0.tar.bz2 ];
then
	wget ftp://ftp.gnu.org/gnu/gcc/gcc-8.3.0/gcc-8.3.0.tar.bz2
fi
...
../gcc-7.1.0/configure --target=$TARGET --prefix="$PREFIX" --enable-languages=c,c++ --with-newlib --enable-threads --with-sysroot=$CROSS_SYSROOT
make all-gcc
